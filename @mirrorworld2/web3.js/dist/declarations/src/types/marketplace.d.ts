...
export interface CreateMarketplacePayload extends Omit<ICreateMarketplacePayload, 'treasury_mint' | 'seller_fee_basis_points'> {
    /**
...
export interface UpdateMarketplacePayload extends Omit<IUpdateMarketplacePayload, 'treasury_mint' | 'seller_fee_basis_points' | 'new_authority'> {
    /**
     * The token in which transactions on the marketplace will occur
     */
    treasuryMint?: string;
...
export declare type MarketplaceQueryOptions = Partial<Pick<Marketplace, 'name' | 'client_id' | 'authority' | 'treasury_mint' | 'auction_house_fee_account' | 'auction_house_treasury' | 'treasury_withdrawal_destination' | 'fee_withdrawal_destination' | 'seller_fee_basis_points' | 'requires_sign_off' | 'can_change_sale_price'>>;
